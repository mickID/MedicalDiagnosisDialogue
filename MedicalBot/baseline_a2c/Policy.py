...
from Utils import test_episode, gather_info

def total_class_num():
    #{0: 30, 1: 34, 3: 33, 2: 45}
    return {0: 30, 1: 34, 3: 33, 2: 45}
    ...
    for epoch in range(1, 1 + max_epoch):
        ...
        with tqdm.tqdm(
            ...
            while t.n < t.total:
                ...
                result = train_collector.collect(n_episode=collect_per_step)  # collect之后就会返回一个result， 即为更新一次所需要收集的数据
                ...
                losses = policy.update(
                    0, train_collector.buffer,  # 训练数据就是collector的buff里面的内容
                    batch_size=batch_size, repeat=repeat_per_collect)
                # print("youxi")
                train_collector.reset_buffer()
                step = max([1] + [
                    len(v) for v in losses.values() if isinstance(v, list)])  # 没太看懂这个step为什么要这样加
                gradient_step += step
        ...
        result = test_episode(policy, test_collector, test_fn, epoch,
                              episode_per_test, writer, env_step)  # 这里能保证遍历所有test集合的数据吗？ 并且需要只遍历一次
        end_time = time.time()
        ...
        if save_fn and best_flag == 1:
            ...
            save_fn(policy)
            # import pickle
            # with open('./model/fuck.pk','wb') as f:
