...
def test_episode(
    ...
    if collector.get_env_num() > 1 and isinstance(n_episode, int):
        ...
        n_[:n_episode % n] += 1
        n_episode = list(n_)
    result = collector.collect(n_episode=n_episode)  # 需要保证这个地方遍历了所有的测试profile
...
#         for k in result.keys():
#             if 'class' not in k:
#                 writer.add_scalar("train_test/" + k, result[k], global_step=global_step)
#     return result
